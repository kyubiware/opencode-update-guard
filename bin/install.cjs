...
function isRegistered(config) {
  return (config.plugin || []).some((p) => typeof p === "string" && p.startsWith(PLUGIN_NAME));
}
...
function register(configPath) {
  ...
  if (isRegistered(config)) {
    console.log("  \u2299 " + PLUGIN_NAME + " already registered in " + path.basename(configPath));
    return;
  }
  ...
  console.log("  \u2713 Registered " + PLUGIN_NAME + " in " + path.basename(configPath));
}
